...
from pico_agent import LLM, DynamicTool, LLMFactory, VirtualAgentManager, VirtualToolManager
...
def test_proto_tool_creation_and_use():
    ...
    tool_manager.create_tool(name="data_processor", description="Processes a list of dictionaries", func=my_logic)

    agent_manager = container.get(VirtualAgentManager)
    agent = agent_manager.create_agent(
        name="worker", system_prompt="Use the tool", tools=["data_processor"], agent_type="react"
    )
